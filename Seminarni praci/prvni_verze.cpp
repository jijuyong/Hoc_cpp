...
void tao_nhan_vat(Character *player){
    SetColor(13);
    char ten_nguoi_choi[50];
    cout << "Zadej jméno svého charakteru: ";
    cin >> ten_nguoi_choi;
    strcpy(player->name, ten_nguoi_choi);
    Sleep(2000);
    xoa_bang(1);
    in_ra_giua("∘₊✧─────✧₊∘",192);
...
int level1(Character *player){
    ...
    string odpoved1;
    getline(cin, odpoved1);
    for (int i = 0; i < odpoved1.length(); i++){
        odpoved1[i] = tolower(odpoved1[i]);
    }
    Sleep(2000);
    xoa_bang(2);
    SetColor(10);
    if (odpoved1 == "peroxid vodiku" || odpoved1 == "peroxid vodíku" || odpoved1 == "peroxidvodiku"){
...
int level2(Character *player){
    ...
    string odpoved2;
    getline(cin, odpoved2);
    for (int i = 0; i < odpoved2.size(); i++){
        odpoved2[i] = tolower(odpoved2[i]);
    }
    if (odpoved2 == "hydroxid sodny" || odpoved2 == "hydroxid sodný" || odpoved2 == "hydroxidsodny"){
        SetColor(10);
        cout << "(👍🏻ᴗ _ᴗ)👍🏻,Správně!" << endl;
        SetColor(7);
        cout << "chcete vidět reakce? (ano/ne): ";
        ...
        if (tra_loi_sai(player) == 1){
            lan_tra_loi_sai++;
            cin.ignore(); // Clear input buffer before restarting
            cout.flush(); // Clear output buffer
            xoa_bang(9);
            cout<<endl;
            level2(player);
    ...
    if (lan_tra_loi_sai == 1 || lan_tra_loi_sai == 2)
        return 1;
    else
        return 0;
}
...
int level3(Character *player){
    ...
    string odpoved1;
    getline(cin, odpoved1);
    for (int i = 0; i < odpoved1.size(); i++){
        odpoved1[i] = tolower(odpoved1[i]);
    }
    if (odpoved1 == "sirouhlik" || odpoved1 == "sirouhlík"){
        SetColor(10);
        cout << "(👍🏻ᴗ _ᴗ)👍🏻,Správně!" << endl;
        SetColor(7);
    }
    ...
    cout << "── .✦Otázka 2✦. ──" << endl;
    SetColor(7);
    cout << "Kolik sirouhlíku se přidává na 1 litr plynu:  ";
    int odpoved2;
    cin >> odpoved2;
    if (odpoved2 == 2){
        SetColor(10);
...
int level5(Character *player){
    ...
    SetColor(14);
    vector<string> luat_choi = {
        "___| |__________________________________________________________________________| |__",
        ...
        "   | |postup                                                                    | |",
        "   | | 1. Bezpečnost: rukavice + brýle                                          | |",
        "   | | 2. Potom nastříkej dva plyny vodík 17:3 kyslík do jednoho balónku     | |",
...
void Hra(Character *player, vector<int> mat_ma, const char *ten_file){
    ...
    if (LVL1 == 0){
        ...
        SetColor(12);
        in_ra_giua("Status: [CHYBA] – Číselný kód uzamčen.", 120);
        SetColor(7);
        so_loi++;
    }

    int LVL2 = level2(player);
    if (LVL2 == 0){
        SetColor(13);
        in_ra_giua("Stíny tentokrát ustoupily tvé vůli. Z hlubin se vynořilo druhé zapomenuté číslo",170);
        Sleep(3000);
    ...
    if (so_loi == 5){
        ...
        SetColor(7);
        int ket_qua = doan_so_zeromistake (mat_ma);
        if (ket_qua==0){
            vyhra(ten_file);
        }else exit(0);
...
int main(){
    ...
    Character nguoi_choi;
    tao_nhan_vat(&nguoi_choi);
    vector<int> mat_ma;
    mat_ma_cua_kho_bau(mat_ma);
    //Hra(&nguoi_choi,mat_ma,"poklad.txt");
    level1(&nguoi_choi);
    return 0;
}